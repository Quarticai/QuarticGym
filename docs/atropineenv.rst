...
What is atropine? We can simply treat it as one medication that benefits human beings. A corresponding continuous prodcution flow can be described as below:
...
is the states and :math:`\hat{x}(t)` is the estimated. The above linear system
...
`SIPPY <https://github.com/CPCLAB-UNIPI/SIPPY>`__. More, since the
identification process requires training data, we can easily generate
the data by the open-loop model.
...
We generated below trajectory with `MPC <https://en.wikipedia.org/wiki/Model_predictive_control>`_, and ended the simulation once it reaches the steady state (we considered this as a termination condition)
...
inputs and the output reach the steady-state inputs and output,
respectively. The whole process (data generation, system identification,
and MPC control) cost less than 10 seconds.
